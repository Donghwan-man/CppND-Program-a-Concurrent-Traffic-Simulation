...
void TrafficLight::waitForGreen()
{
    ...
    while(1) {
        ...
        auto value = _queue.receive();
        if(value == TrafficLightPhase::green) {
            return;
        }
    }
}
...
void TrafficLight::cycleThroughPhases()
{
    ...
    int duration = range(rdmt);

    std::chrono::system_clock::time_point switchTime = std::chrono::system_clock::now();
    
    while(1) {
        auto lightDurationTimeTmp = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::system_clock::now() - switchTime);
        int lightDurationTime = lightDurationTimeTmp.count();

        if(lightDurationTime >= duration) {
            ...
            switchTime = std::chrono::system_clock::now();
            duration = range(rdmt);
        }
